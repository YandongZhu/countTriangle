...
void countNum(long long int* arr, int len){
    bool even = 1;
...
// simple combination for the question only
long long int combination(long long int N) 
{ 
    long long int result = N * (N - 1) * (N - 2);
    return result / 6; 
}

// count the number of invalid case
long long int count(long long int* arr, int len){
    //printarr(A, len);
    long long int sum = 0;
    for (int i = 0; i < len; ++i)
    {
...
int main(int argc, char const *argv[])
{
    ...
    // count all the case each two sum will occur
    countNum(B, 2*n-1);

    // calculate the total invalid case
    long long int invalid = count(B, n);

    // calculate the total number of the sticks
    ...
    // the result will be the combination minus the invalid case
    long long int result = combination(total) - invalid;
